...
class ResidualAttentionBlockANE(ResidualAttentionBlock):
    def __init__(self, n_state: int, n_head: int, cross_attention: bool = False):
        ...
        self.cross_attn = (
            MultiHeadAttentionANE(n_state, n_head) if cross_attention else None
        )
...
class WhisperANE(Whisper):
    ...
    def install_kv_cache_hooks(self, cache: Optional[dict] = None):
        ...
        def save_to_cache(module, _, output):
            if (
                module not in cache
                or output.shape[3] > self.decoder.positional_embedding.shape[0]
            ):
                cache[
                    module
...
def convert_encoder(hparams, model, quantize=False):
    ...
    model = ct.convert(
        ...
        convert_to=None
        if quantize
        else "mlprogram",  # convert will fail if weights are quantized, not sure why
