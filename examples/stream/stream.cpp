...
void whisper_print_usage(int /*argc*/, char **argv,
    ...
    fprintf(stderr,
            ...
            params.length_ms);
    fprintf(stderr,
            "            --keep N        [%-7d] audio to keep from previous "
            "step in ms\n",
            params.keep_ms);
    ...
    fprintf(stderr,
            "  -kc,      --keep-context  [%-7s] keep context between audio "
            "chunks\n",
    ...
    fprintf(stderr,
            "  -sa,      --save-audio    [%-7s] save the recorded audio to a "
            "file\n",
...
int main(int argc, char **argv) {
    ...
    // print some info about the processing
    {
        ...
        fprintf(stderr,
                ...
                "keep = %.1f sec), %d threads, lang = %s, task = %s, "
                "timestamps = %d ...\n",
    ...
    while (is_running) {
        ...
        // run the inference
        {
            ...
            wparams.prompt_n_tokens =
                params.no_context ? 0 : prompt_tokens.size();
            ...
            // print result;
            {
                if (!use_vad) {
                    ...
                    printf("\n");
                    printf("### Transcription %d START | t0 = %d ms | t1 = %d "
                           "ms\n",
                ...
                for (int i = 0; i < n_segments; ++i) {
                    ...
                    if (params.no_timestamps) {
                        ...
                        std::string output = "[" + to_timestamp(t0) + " --> " +
                                             to_timestamp(t1) + "]  " + text;

                        if (whisper_full_get_segment_speaker_turn_next(ctx,
                                                                       i)) {
                            output += " [SPEAKER_TURN]";
                        }
            ...
            if (!use_vad && (n_iter % n_new_line) == 0) {
                ...
                pcmf32_old = std::vector<float>(pcmf32.end() - n_samples_keep,
                                                pcmf32.end());
