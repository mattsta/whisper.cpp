...
size_t inputStreamRead(void *ctx, void *output, size_t read_size) {
    ...
    jint avail_size =
        (*is->env)->CallIntMethod(is->env, is->input_stream, is->mid_available);
    jint size_to_copy = read_size < avail_size ? (jint)read_size : avail_size;

    jbyteArray byte_array = (*is->env)->NewByteArray(is->env, size_to_copy);

    jint n_read = (*is->env)->CallIntMethod(
        is->env, is->input_stream, is->mid_read, byte_array, 0, size_to_copy);

    if (size_to_copy != read_size || size_to_copy != n_read) {
        LOGI("Insufficient Read: Req=%zu, ToCopy=%d, Available=%d", read_size,
             size_to_copy, n_read);
    }

    jbyte *byte_array_elements =
        (*is->env)->GetByteArrayElements(is->env, byte_array, NULL);
    memcpy(output, byte_array_elements, size_to_copy);
    (*is->env)->ReleaseByteArrayElements(is->env, byte_array,
...
static struct whisper_context *whisper_init_from_asset(JNIEnv *env,
                                                       jobject assetManager,
                                                       const char *asset_path) {
    LOGI("Loading model from asset '%s'\n", asset_path);
    AAssetManager *asset_manager = AAssetManager_fromJava(env, assetManager);
    AAsset *asset =
        AAssetManager_open(asset_manager, asset_path, AASSET_MODE_STREAMING);
    if (!asset) {
        LOGW("Failed to open '%s'\n", asset_path);
        return NULL;
    }

    whisper_model_loader loader = {.context = asset,
                                   .read = &asset_read,
                                   .eof = &asset_is_eof,
                                   .close = &asset_close};
...
Java_com_whispercppdemo_whisper_WhisperLib_00024Companion_initContextFromAsset(
    ...
    UNUSED(thiz);
    struct whisper_context *context = NULL;
    const char *asset_path_chars =
        (*env)->GetStringUTFChars(env, asset_path_str, NULL);
...
Java_com_whispercppdemo_whisper_WhisperLib_00024Companion_initContext(
    ...
    const char *model_path_chars =
        (*env)->GetStringUTFChars(env, model_path_str, NULL);
...
Java_com_whispercppdemo_whisper_WhisperLib_00024Companion_fullTranscribe(
    ...
    jfloat *audio_data_arr =
        (*env)->GetFloatArrayElements(env, audio_data, NULL);
    ...
    (*env)->ReleaseFloatArrayElements(env, audio_data, audio_data_arr,
                                      JNI_ABORT);
}
...
JNIEXPORT jstring JNICALL
Java_com_whispercppdemo_whisper_WhisperLib_00024Companion_benchMemcpy(
...
JNIEXPORT jstring JNICALL
Java_com_whispercppdemo_whisper_WhisperLib_00024Companion_benchGgmlMulMat(
    JNIEnv *env, jobject thiz, jint n_threads) {
    UNUSED(thiz);
