...
void whisper_print_usage(int /*argc*/, char **argv,
    ...
    fprintf(stderr,
            ...
            params.n_threads);
    fprintf(stderr,
            "  -p N,      --processors N      [%-7d] number of processors to "
    ...
    fprintf(stderr,
            "  -lpt N,    --logprob-thold N   [%-7.2f] log probability "
            "threshold for decoder fail\n",
            params.logprob_thold);
    // fprintf(stderr, "  -su,       --speed-up          [%-7s] speed up audio
    // by x2 (reduced accuracy)\n",        params.speed_up ? "true" : "false");
    fprintf(stderr,
            "  -debug,    --debug-mode        [%-7s] enable debug mode (eg. "
            "dump log_mel)\n",
    ...
    fprintf(stderr,
            "  -otxt,     --output-txt        [%-7s] output result in a text "
            "file\n",
            params.output_txt ? "true" : "false");
    fprintf(
        stderr,
        "  -ovtt,     --output-vtt        [%-7s] output result in a vtt file\n",
        params.output_vtt ? "true" : "false");
    fprintf(
        stderr,
        "  -osrt,     --output-srt        [%-7s] output result in a srt file\n",
        params.output_srt ? "true" : "false");
    fprintf(
        stderr,
        "  -olrc,     --output-lrc        [%-7s] output result in a lrc file\n",
    ...
    fprintf(
        stderr,
        "  -ocsv,     --output-csv        [%-7s] output result in a CSV file\n",
        params.output_csv ? "true" : "false");
    fprintf(stderr,
            "  -oj,       --output-json       [%-7s] output result in a JSON "
            "file\n",
    ...
    fprintf(stderr,
            "  -l LANG,   --language LANG     [%-7s] spoken language ('auto' "
            "for auto-detect)\n",
    ...
    fprintf(stderr, "  -m FNAME,  --model FNAME       [%-7s] model path\n",
            params.model.c_str());
    fprintf(stderr,
            "  -f FNAME,  --file FNAME        [%-7s] input WAV file path\n",
            "");
    fprintf(stderr,
    ...
    fprintf(stderr,
            "  -ls,       --log-score         [%-7s] log best decoder scores "
            "of tokens\n",
...
void whisper_print_progress_callback(struct whisper_context * /*ctx*/,
    ...
    int *progress_prev =
        &(((whisper_print_user_data *)user_data)->progress_prev);
...
bool output_csv(struct whisper_context *ctx, const char *fname,
    ...
    for (int i = 0; i < n_segments; ++i) {
        ...
        // need to multiply times returned from
        // whisper_full_get_segment_t{0,1}() by 10 to get milliseconds.
        fout << 10 * t0 << "," << 10 * t1 << ",";
        if (params.diarize && pcmf32s.size() == 2) {
...
bool output_score(struct whisper_context *ctx, const char *fname,
                  const whisper_params & /*params*/,
                  std::vector<std::vector<float>> /*pcmf32s*/) {
    std::ofstream fout(fname);
    fprintf(stderr, "%s: saving output to '%s'\n", __func__, fname);
...
bool output_json(struct whisper_context *ctx, const char *fname,
    ...
    for (int i = 0; i < n_segments; ++i) {
        ...
        if (params.diarize && pcmf32s.size() == 2) {
            value_s("speaker",
                    estimate_diarization_speaker(pcmf32s, t0, t1, true).c_str(),
                    true);
        }

        if (params.tinydiarize) {
            value_b("speaker_turn_next",
...
bool output_wts(struct whisper_context *ctx, const char *fname,
                const char *fname_inp, const whisper_params &params,
    ...
    if (!fin.is_open()) {
        fprintf(stderr,
                "%s: font not found at '%s', please specify a monospace font "
                "with -fp\n",
                __func__, font);
        return false;
    }
    ...
    fout << "ffmpeg -i " << fname_inp
         << " -f lavfi -i color=size=1200x120:duration=" << t_sec
    ...
    for (int i = 0; i < whisper_full_n_segments(ctx); i++) {
        ...
        fout << "drawtext=fontfile='" << font
             << "':fontsize=24:fontcolor=gray:x=(w-text_w)/2:y=h/"
                "2:text='':enable='between(t,"
             << t0 / 100.0 << "," << t0 / 100.0 << ")'";
        ...
        for (int j = 0; j < n; ++j) {
...

            {
                for (int k = 0; k < n; ++k) {
                    ...
                    const std::string txt =
                        whisper_token_to_str(ctx, token2.id);
            ...
            if (is_first) {
                ...
                fout << ",drawtext=fontfile='" << font
                     << "':fontsize=24:fontcolor=gray:x=(w-text_w)/2:y=h/"
                        "2:text='"
                     << txt_bg << "':enable='between(t," << t0 / 100.0 << ","
                     << t1 / 100.0 << ")'";
                is_first = false;
            }
            ...
            // foreground text
            fout << ",drawtext=fontfile='" << font
                 << "':fontsize=24:fontcolor=lightgreen:x=(w-text_w)/2+8:y=h/"
                    "2:text='"
                 << txt_fg << "':enable='between(t," << token.t0 / 100.0 << ","
                 << token.t1 / 100.0 << ")'";
            ...
            fout << ",drawtext=fontfile='" << font
                 << "':fontsize=24:fontcolor=lightgreen:x=(w-text_w)/2+8:y=h/"
                    "2+16:text='"
                 << txt_ul << "':enable='between(t," << token.t0 / 100.0 << ","
                 << token.t1 / 100.0 << ")'";
        }
    }
...
int main(int argc, char **argv) {
    ...
    // initialize openvino encoder. this has no effect on whisper.cpp builds
    ...
    for (int f = 0; f < (int)params.fname_inp.size(); ++f) {
        ...
        // print some info about the processing
        {
            ...
            fprintf(
                stderr,
                "%s: processing '%s' (%d samples, %.1f sec), %d threads, %d "
                "processors, lang = %s, task = %s, %stimestamps = %d ...\n",
                __func__, fname_inp.c_str(), int(pcmf32.size()),
                float(pcmf32.size()) / WHISPER_SAMPLE_RATE, params.n_threads,
                params.n_processors, params.language.c_str(),
        ...
        // run the inference
        {
            ...
            // in examples below, we do not abort the processing, but we could
            // if the flag is set to true

            // the callback is called before every encoder run - if it returns
            // false, the processing is aborted
            {
            ...
            // the callback is called before every computation - if it returns
            // true, the computation is aborted
            {
                static bool is_aborted =
                    false; // NOTE: this should be atomic to avoid data race
            ...
            if (whisper_full_parallel(ctx, wparams, pcmf32.data(),
                                      pcmf32.size(),
        ...
        // output stuff
        {
            ...
            if (params.output_wts) {
                ...
                output_wts(ctx, fname_wts.c_str(), fname_inp.c_str(), params,
                           float(pcmf32.size() + 1000) / WHISPER_SAMPLE_RATE,
                           pcmf32s);
            }
