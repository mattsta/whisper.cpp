...
void whisper_print_usage(int /*argc*/, char **argv,
    ...
    fprintf(stderr, "  -s FILE,  --speak TEXT    [%-7s] command for TTS\n",
            params.speak.c_str());
    fprintf(stderr,
            "  --prompt-file FNAME       [%-7s] file with custom prompt to "
            "start dialog\n",
            "");
    fprintf(stderr, "  --session FNAME       file to cache model state in (may "
                    "be large!) (default: none)\n");
    fprintf(stderr,
...
int main(int argc, char **argv) {
    ...
    struct whisper_context *ctx_wsp =
        whisper_init_from_file(params.model_wsp.c_str());
    ...
    struct llama_model *model_llama =
        llama_load_model_from_file(params.model_llama.c_str(), lparams);

    struct llama_context *ctx_llama =
        llama_new_context_with_model(model_llama, lparams);
    ...
    const std::string prompt_whisper =
        ::replace(k_prompt_whisper, "{1}", bot_name);

    // construct the initial prompt for LLaMA inference
    std::string prompt_llama =
        params.prompt.empty() ? k_prompt_llama : params.prompt;
    ...
    if (!path_session.empty()) {
        ...
        if (fp != NULL) {
            ...
            fprintf(stderr,
                    "%s: loaded a session with prompt size of %d tokens\n",
    ...
    if (session_tokens.size()) {
        ...
        if (n_matching_session_tokens >= embd_inp.size()) {
            ...
            fprintf(stderr,
                    "%s: warning: session file has low similarity to prompt "
                    "(%zu / %zu tokens); will mostly be reevaluated\n",
                    __func__, n_matching_session_tokens, embd_inp.size());
        } else {
            fprintf(stderr,
                    "%s: session file matches %zu / %zu tokens of prompt\n",
                    __func__, n_matching_session_tokens, embd_inp.size());
        }
    }
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1250,
                ...
                if (!force_speak) {
                    text_heard =
                        ::trim(::transcribe(ctx_wsp, params, pcmf32_cur,
                ...
                // take first line
                text_heard =
                    text_heard.substr(0, text_heard.find_first_of('\n'));

                // remove leading and trailing whitespace
                text_heard =
                    std::regex_replace(text_heard, std::regex("^\\s+"), "");
                text_heard =
                    std::regex_replace(text_heard, std::regex("\\s+$"), "");

                const std::vector<llama_token> tokens =
                    llama_tokenize(ctx_llama, text_heard.c_str(), false);
                ...
                if (!path_session.empty()) {
                    session_tokens.insert(session_tokens.end(), tokens.begin(),
                                          tokens.end());
                }
                ...
                while (true) {
                    ...
                    if (embd.size() > 0) {
                        if (n_past + (int)embd.size() > n_ctx) {
                            ...
                            // insert n_left/2 tokens at the start of embd from
                            // last_n_tokens
                            embd.insert(embd.begin(),
                                        embd_inp.begin() + embd_inp.size() -
                                            n_prev,
                                        embd_inp.end());
                            ...
                            //     printf("%s", llama_token_to_piece(ctx_llama,
                            //     embd[i]));
                            // }
                            // printf("'\n");
                            // printf("\n---\n");
                        ...
                        // try to reuse a matching prefix from the loaded
                        // session instead of re-eval (via n_past) REVIEW
                        if (n_session_consumed < (int)session_tokens.size()) {
                            size_t i = 0;
                            for (; i < embd.size(); i++) {
                                if (embd[i] !=
                                    session_tokens[n_session_consumed]) {
                        ...
                        if (llama_eval(ctx_llama, embd.data(), embd.size(),
...

                    {
...

                        {
                            ...
                            const float nl_logit =
                                logits[llama_token_nl(ctx_llama)];

                            llama_sample_repetition_penalty(
                                ctx_llama, &candidates_p,
                                embd_inp.data() +
                                    std::max(0, n_past - repeat_last_n),
                            ...
                            if (temp <= 0) {
                                // Greedy sampling
                                id = llama_sample_token_greedy(ctx_llama,
                                                               &candidates_p);
                            } else {
                                // Temperature sampling
                                llama_sample_top_k(ctx_llama, &candidates_p,
                                                   top_k, 1);
                                llama_sample_top_p(ctx_llama, &candidates_p,
                                                   top_p, 1);
                                llama_sample_temperature(ctx_llama,
                                                         &candidates_p, temp);
                                id = llama_sample_token(ctx_llama,
                                                        &candidates_p);
                            }
                        }
                        ...
                        if (id != llama_token_eos(ctx_llama)) {
                            ...
                            text_to_speak +=
                                llama_token_to_piece(ctx_llama, id);

                            printf("%s",
                                   llama_token_to_piece(ctx_llama, id).c_str());
                        }
                    }

                    {
                        std::string last_output;
                        for (int i = embd_inp.size() - 16;
                             i < (int)embd_inp.size(); i++) {
                            last_output +=
                ...
                int ret =
                    system((params.speak + " " + std::to_string(voice_id) +
                            " \"" + text_to_speak + "\"")
                               .c_str());
                if (ret != 0) {
