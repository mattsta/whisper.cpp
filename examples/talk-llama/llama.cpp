...
static void llama_log_callback_default(llama_log_level level, const char *text,
                                       void *user_data);

#define LLAMA_LOG_INFO(...)                                                    \
    llama_log_internal(LLAMA_LOG_LEVEL_INFO, __VA_ARGS__)
#define LLAMA_LOG_WARN(...)                                                    \
    llama_log_internal(LLAMA_LOG_LEVEL_WARN, __VA_ARGS__)
#define LLAMA_LOG_ERROR(...)                                                   \
    llama_log_internal(LLAMA_LOG_LEVEL_ERROR, __VA_ARGS__)
...
//   std::string name = tn(LLM_TENSOR_TOKEN_EMBD, "bias");         ->
//   "token_embd.bias" std::string name = tn(LLM_TENSOR_ATTN_NORM, "weight", 3);
...
struct LLM_TN {
    ...
    std::string operator()(llm_tensor tensor, const std::string &suffix,
                           int bid) const {
        return ::format(LLM_TENSOR_NAMES[arch].at(tensor).c_str(), bid) + "." +
               suffix;
    }
...
#define GGUF_GET_KEY(ctx, dst, func, type, req, key)                           \
    {                                                                          \
        ...
        if (kid >= 0) {                                                        \
            ...
            if (ktype != (type)) {                                             \
                throw std::runtime_error(format("key %s has wrong type: %s",   \
                                                skey.c_str(),                  \
...
struct llama_file {
    ...
    llama_file(const char *fname, const char *mode) {
        ...
        if (fp == NULL) {
            throw std::runtime_error(
                format("failed to open %s: %s", fname, strerror(errno)));
        }
        seek(0, SEEK_END);
        size = tell();
        seek(0, SEEK_SET);
    ...
    void read_raw(void *ptr, size_t len) const {
        ...
        if (ret != 1) {
            throw std::runtime_error(
                std::string("unexpectedly reached end of file"));
        }
    }
    ...
    void write_raw(const void *ptr, size_t len) const {
        ...
        if (ret != 1) {
            throw std::runtime_error(
                format("write error: %s", strerror(errno)));
        }
    }
...
struct llama_mmap {
    ...
    llama_mmap(struct llama_file *file,
        ...
        if (addr == MAP_FAILED) {
            throw std::runtime_error(
                format("mmap failed: %s", strerror(errno)));
        }
        ...
        if (prefetch > 0) {
            ...
            if (posix_madvise(addr, std::min(file->size, prefetch),
                              POSIX_MADV_WILLNEED)) {
                fprintf(stderr,
                        "warning: posix_madvise(.., POSIX_MADV_WILLNEED) "
                        "failed: %s\n",
                        strerror(errno));
            }
        }
        if (numa) {
            ...
            if (posix_madvise(addr, file->size, POSIX_MADV_RANDOM)) {
                fprintf(stderr,
                        "warning: posix_madvise(.., POSIX_MADV_RANDOM) failed: "
                        "%s\n",
                        strerror(errno));
            }
        }
    ...
    llama_mmap(struct llama_file *file, bool prefetch = true,
        ...
        if (hMapping == NULL) {
            throw std::runtime_error(
                format("CreateFileMappingA failed: %s",
        ...
        if (addr == NULL) {
            throw std::runtime_error(
                format("MapViewOfFile failed: %s",
                       llama_format_win_err(error).c_str()));
        }

        if (prefetch) {
            // PrefetchVirtualMemory is only present on Windows 8 and above, so
            ...
            if (pPrefetchVirtualMemory) {
                ...
                if (!pPrefetchVirtualMemory(GetCurrentProcess(), 1, &range,
                                            0)) {
                    fprintf(stderr,
...
static std::string llama_token_to_str(const struct llama_context *ctx,
                                      llama_token token) {
...
struct llama_context {
    llama_context(const llama_model &model)
        : model(model), t_load_us(model.t_load_us),
    ...
    int32_t n_p_eval = 0; // number of tokens in eval calls for the prompt (with
                          // batch size > 1)
...
static bool llama_kv_cache_init(const struct llama_hparams &hparams,
...
struct llama_model_loader {
    ...
    llama_model_loader(const std::string &fname, bool use_mmap)
        ...
        LLAMA_LOG_INFO("%s: loaded meta data with %d key-value pairs and %d "
                       "tensors from %s (version %s)\n",
        ...
        // quantization and print meta data
        // TODO: make optional
        {
            ...
            for (int i = 0; i < n_tensors; i++) {
                ...
                LLAMA_LOG_INFO("%s: - tensor %4d: %32s %-8s [ %s ]\n", __func__,
                               i, name, ggml_type_name(meta->type),
    ...
    std::string get_arch_name() const {
        ...
        GGUF_GET_KEY(ctx_gguf, arch_name, gguf_get_val_str, GGUF_TYPE_STRING,
                     false, kv(LLM_KV_GENERAL_ARCHITECTURE));
    ...
    struct ggml_tensor *create_tensor_for(struct ggml_context *ctx,
    ...
    struct ggml_tensor *create_tensor(struct ggml_context *ctx,
        ...
        if (cur == NULL) {
            throw std::runtime_error(
                format("%s: tensor '%s' not found", __func__, name.c_str()));
        }
    ...
    void done_getting_tensors() const {
        if (n_created != n_tensors) {
            throw std::runtime_error(
                format("%s: wrong number of tensors; expected %d, got %d",
    ...
    size_t file_offset(const char *name) const {
        ...
        if (idx < 0) {
            throw std::runtime_error(format(
                "%s: tensor '%s' not found in the file", __func__, name));
        }

        return gguf_get_data_offset(ctx_gguf) +
               gguf_get_tensor_offset(ctx_gguf, idx);
    }
    ...
    void load_all_data(struct ggml_context *ctx,
        ...
        for (int i = 0; i < gguf_get_n_tensors(ctx_gguf); i++) {
            struct ggml_tensor *cur =
                ggml_get_tensor(ctx, gguf_get_tensor_name(ctx_gguf, i));
        ...
        for (int i = 0; i < gguf_get_n_tensors(ctx_gguf); i++) {
            struct ggml_tensor *cur =
                ggml_get_tensor(ctx, gguf_get_tensor_name(ctx_gguf, i));
            GGML_ASSERT(cur); // unused tensors should have been caught by
                              // load_data already

            if (progress_callback) {
                progress_callback((float)done_size / size_data,
...
std::string llama_model_ftype_name(enum llama_ftype ftype) {
    if (ftype & LLAMA_FTYPE_GUESSED) {
        return llama_model_ftype_name(
                   (enum llama_ftype)(ftype & ~LLAMA_FTYPE_GUESSED)) +
               " (guessed)";
    }
...
static void llm_load_hparams(llama_model_loader &ml, llama_model &model,
                             int n_ctx, float rope_freq_base,
    ...
    GGUF_GET_KEY(ctx, hparams.n_head_kv, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_ATTENTION_HEAD_COUNT_KV));
    ...
    // sanity check for n_rot (optional)
    {
        ...
        GGUF_GET_KEY(ctx, hparams.n_rot, gguf_get_val_u32, GGUF_TYPE_UINT32,
                     false, kv(LLM_KV_ROPE_DIMENSION_COUNT));

        if (model.arch == LLM_ARCH_LLAMA || model.arch == LLM_ARCH_FALCON) {
            if (hparams.n_rot != hparams.n_embd / hparams.n_head) {
                throw std::runtime_error(
                    format("invalid n_rot: %u, expected %u", hparams.n_rot,
    ...
    switch (model.arch) {
    case LLM_ARCH_LLAMA: {
        GGUF_GET_KEY(ctx, hparams.f_norm_rms_eps, gguf_get_val_f32,
                     GGUF_TYPE_FLOAT32, true,
        ...
        case 60:
            ...
            break;
        case 80:
            model.type = hparams.n_head == hparams.n_head_kv
                             ? e_model::MODEL_65B
                             : e_model::MODEL_70B;
            break;
        default:
    ...
    case LLM_ARCH_FALCON: {
        GGUF_GET_KEY(ctx, hparams.f_norm_eps, gguf_get_val_f32,
                     GGUF_TYPE_FLOAT32, true,
...
static void llm_load_vocab(llama_model_loader &ml, llama_model &model) {
    ...
    // determine vocab type
    {
        ...
        GGUF_GET_KEY(ctx, tokenizer_name, gguf_get_val_str, GGUF_TYPE_STRING,
                     true, kv(LLM_KV_TOKENIZER_MODEL));
        ...
        if (tokenizer_name == "llama") {
            ...
            LLAMA_LOG_WARN("%s: unknown tokenizer: '%s'", __func__,
    ...
    // special tokens
    GGUF_GET_KEY(ctx, vocab.special_bos_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_TOKENIZER_BOS_ID));
    GGUF_GET_KEY(ctx, vocab.special_eos_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_TOKENIZER_EOS_ID));
    GGUF_GET_KEY(ctx, vocab.special_unk_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_TOKENIZER_UNK_ID));
    GGUF_GET_KEY(ctx, vocab.special_sep_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_TOKENIZER_SEP_ID));
    GGUF_GET_KEY(ctx, vocab.special_pad_id, gguf_get_val_u32, GGUF_TYPE_UINT32,
                 false, kv(LLM_KV_TOKENIZER_PAD_ID));
}
...
static void llm_load_print_meta(llama_model_loader &ml, llama_model &model) {
    ...
    LLAMA_LOG_INFO("%s: format         = %s\n", __func__,
                   llama_file_version_name(ml.fver));
    LLAMA_LOG_INFO("%s: arch           = %s\n", __func__,
                   LLM_ARCH_NAMES.at(model.arch).c_str());
    LLAMA_LOG_INFO("%s: vocab type     = %s\n", __func__,
                   vocab.type == LLAMA_VOCAB_TYPE_SPM ? "SPM"
                                                      : "BPE"); // TODO: fix
    LLAMA_LOG_INFO("%s: n_vocab        = %u\n", __func__, hparams.n_vocab);
    LLAMA_LOG_INFO("%s: n_merges       = %u\n", __func__,
                   (int)vocab.bpe_ranks.size());
...
static void llm_load_tensors(llama_model_loader &ml, llama_model &model,
                             int n_batch, int n_gpu_layers, int main_gpu,
                             const float *tensor_split, const bool mul_mat_q,
                             bool low_vram, ggml_type memory_type,
                             bool use_mlock,
                             llama_progress_callback progress_callback,
...
#define LLAMA_BACKEND_OFFLOAD_SPLIT GGML_BACKEND_CPU
    ...
    size_t vram_weights = 0;
    {
        ...
        case LLM_ARCH_LLAMA: {
            model.tok_embeddings =
                ml.create_tensor(ctx, tn(LLM_TENSOR_TOKEN_EMBD, "weight"),
            ...
            // output
            {
                ...
                if (n_gpu_layers > int(n_layer)) {
                    // norm is not performance relevant on its own but keeping
                    // it in VRAM reduces data copying on Windows however this
                    // is detrimental unless everything is on the GPU
#ifndef _WIN32
                    backend_norm =
                        low_vram ? GGML_BACKEND_CPU : LLAMA_BACKEND_OFFLOAD;
#else
                ...
                model.output_norm =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT_NORM, "weight"),
                                     {n_embd}, backend_norm);
                model.output =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT, "weight"),
            ...
            for (uint32_t i = 0; i < n_layer; ++i) {
                ...
                layer.attn_norm =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_NORM, "weight", i),
                                     {n_embd}, backend);

                layer.wq =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_Q, "weight", i),
                                     {n_embd, n_embd}, backend_split);
                layer.wk =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_K, "weight", i),
                                     {n_embd, n_embd_gqa}, backend_split);
                layer.wv =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_V, "weight", i),
                                     {n_embd, n_embd_gqa}, backend_split);
                layer.wo =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_OUT, "weight", i),
                                     {n_embd, n_embd}, backend_split);

                layer.ffn_norm =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_NORM, "weight", i),
                                     {n_embd}, backend);

                layer.w1 =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_GATE, "weight", i),
                                     {n_embd, n_ff}, backend_split);
                layer.w2 =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_DOWN, "weight", i),
                                     {n_ff, n_embd}, backend_split);
                layer.w3 =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_FFN_UP, "weight", i),
                                     {n_embd, n_ff}, backend_split);
            ...
            // output
            {
                ...
                model.output_norm =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT_NORM, "weight"),
                                     {n_embd}, backend_norm);
                model.output_norm_b =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT_NORM, "bias"),
                                     {n_embd}, backend_norm);
                model.output =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_OUTPUT, "weight"),
                                     {n_embd, n_vocab}, backend_output);
            ...
            for (uint32_t i = 0; i < n_layer; ++i) {
                ...
                layer.attn_norm =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_NORM, "weight", i),
                                     {n_embd}, backend);
                layer.attn_norm_b =
                    ml.create_tensor(ctx, tn(LLM_TENSOR_ATTN_NORM, "bias", i),
                                     {n_embd}, backend);
                ...
                if (gguf_find_tensor(
                    ...
                    layer.attn_norm_2 = ml.create_tensor(
                        ctx, tn(LLM_TENSOR_ATTN_NORM_2, "weight", i), {n_embd},
                        backend);
                    layer.attn_norm_2_b = ml.create_tensor(
                        ctx, tn(LLM_TENSOR_ATTN_NORM_2, "bias", i), {n_embd},
                        backend);
                ...
                layer.wqkv = ml.create_tensor(
                    ctx, tn(LLM_TENSOR_ATTN_QKV, "weight", i),
    ...
    // print memory requirements
    {
        ...
        if (n_gpu_layers > (int)hparams.n_layer + 1) {
            if (low_vram) {
                LLAMA_LOG_INFO("%s: cannot offload v cache to GPU due to low "
                               "VRAM option\n",
                               __func__);
            } else {
        ...
        if (n_gpu_layers > (int)hparams.n_layer + 2) {
            if (low_vram) {
                LLAMA_LOG_WARN("%s: cannot offload k cache to GPU due to low "
                               "VRAM option\n",
                               __func__);
            } else {
        ...
        LLAMA_LOG_INFO("%s: VRAM used: %zu MB\n", __func__,
                       (vram_weights + vram_kv_cache + MB - 1) /
                           MB); // round up
#else
        (void)n_gpu_layers;
...
static bool llama_model_load(const std::string &fname, llama_model &model,
                             int n_ctx, int n_batch, int n_gpu_layers,
                             int main_gpu, const float *tensor_split,
                             const bool mul_mat_q, float rope_freq_base,
                             float rope_freq_scale, bool low_vram,
    ...
    try {
        ...
        llm_load_tensors(*ml, model, n_batch, n_gpu_layers, main_gpu,
                         tensor_split, mul_mat_q, low_vram, memory_type,
                         use_mlock, progress_callback,
                         progress_callback_user_data);
...
static struct ggml_cgraph *llm_build_llama(llama_context &lctx,
        ...
        // self-attention
        {
            ...
            // store key and value to memory
            {
                ...
                struct ggml_tensor *Vcur = ggml_transpose(
                    ctx0, ggml_reshape_2d(ctx0, tmpv, n_embd_gqa, N));
                offload_func_v(Vcur);
            ...
            struct ggml_tensor *K = ggml_view_3d(
                ctx0, kv_self.k, n_embd_head, n_past + N, n_head_kv,
                ggml_element_size(kv_self.k) * n_embd_gqa,
            ...
            struct ggml_tensor *KQ_scaled =
                ggml_scale_inplace(ctx0, KQ, KQ_scale);
            ...
            struct ggml_tensor *KQ_soft_max =
                ggml_soft_max_inplace(ctx0, KQ_masked);
            offload_func_v(KQ_soft_max);
            ggml_set_name(KQ_soft_max, "KQ_soft_max");

            // split cached V into n_head heads
            struct ggml_tensor *V = ggml_view_3d(
                ctx0, kv_self.v, n_past + N, n_embd_head, n_head_kv,
...
#else
            // make V contiguous in memory to speed up the matmul, however we
            // waste time on the copy on M1 this is faster for the perplexity
            // computation, but ~5% slower for the single-token generation is
            // there a better way?
...
static struct ggml_cgraph *llm_build_baichaun(llama_context &lctx,
                                              const llama_token *tokens,
                                              const float *embd, int n_tokens,
                                              int n_past) {
    GGML_ASSERT((!tokens && embd) || (tokens && !embd)); // NOLINT
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            // KQ_scaled, n_past); struct ggml_tensor * KQ_masked =
...
static struct ggml_cgraph *llm_build_falcon(llama_context &lctx,
                                            const llama_token *tokens,
                                            const float *embd, int n_tokens,
                                            int n_past) {
    GGML_ASSERT((!tokens && embd) || (tokens && !embd)); // NOLINT
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // TODO: refactor into common function (shared with LLaMA)
        {
            ...
            attn_norm = ggml_add(
                ctx0, ggml_mul(ctx0, attn_norm, model.layers[il].attn_norm),
            ...
            if (model.layers[il].attn_norm_2) { // Falcon-40B
                ...
                cur = ggml_add(
                    ctx0, ggml_mul(ctx0, cur, model.layers[il].attn_norm_2),
            ...
            // TODO: these 2 ggml_conts are technically not needed, but we add
            // them until CUDA support for
            //       non-contiguous views is added for the rope operator
            struct ggml_tensor *tmpq = ggml_cont(
                ctx0,
                ggml_view_3d(
                    ctx0, cur, n_embd_head, n_head, N, wsize * n_embd_head,
                    wsize * n_embd_head * (n_head + 2 * n_head_kv), 0));
            offload_func_kq(tmpq);

            struct ggml_tensor *tmpk =
                ggml_cont(ctx0, ggml_view_3d(ctx0, cur, n_embd_head, n_head_kv,
                                             N, wsize * n_embd_head,
                                             wsize * n_embd_head *
                                                 (n_head + 2 * n_head_kv),
            ...
            struct ggml_tensor *tmpv = ggml_view_3d(
                ctx0, cur, n_embd_head, n_head_kv, N, wsize * n_embd_head,
            ...
            {
                struct ggml_tensor *Vcur = ggml_transpose(
                    ctx0, ggml_reshape_2d(ctx0, ggml_cont(ctx0, tmpv),
                                          n_embd_gqa, N));
                offload_func_v(Vcur);
    ...
    // norm
    {
        ...
        cur = ggml_add(ctx0, ggml_mul(ctx0, cur, model.output_norm),
                       model.output_norm_b);
        ggml_set_name(cur, "result_norm");
    }
...
static bool llama_eval_internal(llama_context &lctx, const llama_token *tokens,
    ...
    for (int i = 0; i < gf->n_leafs; i++) {
        ...
        if (node->backend == GGML_BACKEND_GPU && node->extra == NULL) {
            ggml_cuda_assign_scratch_offset(
                node, (char *)node->data - (char *)lctx.buf_alloc.data);
        }
    }
    ...
    // TODO: this is mostly important for Apple Silicon where CBLAS is still
    // performing very well
    //       we still need some threads to process all non-mul_mat ops, but not
    //       too much to avoid interfering with the BLAS calls. need a better
    //       solution
        ...
        if (p == rev_merge.end()) {
            ...
            for (int j = 0; j < (int)symbol.n; ++j) {
                llama_vocab::id token_id =
                    llama_byte_to_token(vocab, symbol.text[j]);
...
// TODO: there are a lot of common parts between spm and bpe tokenizers, should
// be refactored and reused

struct llm_bigram_bpe {
    struct comparator {
        bool operator()(const llm_bigram_bpe &l,
                        const llm_bigram_bpe &r) const {
    ...
    using queue =
        std::priority_queue<llm_bigram_bpe, queue_storage, comparator>;
    llm_symbol::index left;
    llm_symbol::index right;
    std::string text;
    int rank;
...
struct llm_tokenizer_bpe {
    ...
    void tokenize(const std::string &text,
        ...
        for (auto &word : word_collection) {
            ...
            while (offset < word.size()) {
                llm_symbol sym;
                size_t char_len = std::min(word.size() - offset,
                                           (size_t)::utf8_len(word[offset]));
            ...
            // add the fnished tokens to the final list keeping correct order
            // for next and prev
                        ...
                        if (token_multibyte == vocab.token_to_id.end()) {
                            try {
                                llama_token token_byte =
                                    llama_byte_to_token(vocab, *j);
                                output.push_back(token_byte);
                            } catch (const std::out_of_range &err) {
                                fprintf(
                                    stderr,
                                    "ERROR: byte not found in vocab: '%s'\n",
                                    byte_str.c_str());
                            }
                        } else {
  ...
  private:
    ...
    bpe_gpt2_preprocess(const std::string &text) {
...
llama_tokenize_internal(const llama_vocab &vocab, std::string raw_text,
    ...
    case LLAMA_VOCAB_TYPE_SPM: {
        // without adding this leading whitespace, we do not get the same
        // results as the original tokenizer
        raw_text = " " + raw_text;
...
std::pair<std::vector<uint32_t>, llama_partial_utf8>
decode_utf8(const char *src, llama_partial_utf8 partial_start) {
    ...
    while (*pos != 0 && n_remain > 0) {
        ...
        if ((next_byte >> 6) != 2) {
            ...
            return std::make_pair(std::move(code_points),
                                  llama_partial_utf8{0, -1});
        }
    ...
    return std::make_pair(std::move(code_points),
                          llama_partial_utf8{value, n_remain});
}
...
static std::pair<bool, const llama_grammar_element *>
llama_grammar_match_char(const llama_grammar_element *pos, const uint32_t chr) {
    bool found = false;
    bool is_positive_char = pos->type == LLAMA_GRETYPE_CHAR;

    GGML_ASSERT(is_positive_char ||
                pos->type == LLAMA_GRETYPE_CHAR_NOT); // NOLINT
...
static bool
llama_grammar_match_partial_char(const llama_grammar_element *pos,
    ...
    // range of possible code points this partial UTF-8 sequence could complete
    // to
    ...
    case LLAMA_GRETYPE_RULE_REF: {
        ...
        do {
            ...
            if (!llama_grammar_is_end_of_sequence(pos + 1)) {
                // if this rule ref is followed by another element, add that to
                // stack
    ...
    for (auto tok : candidates) {
        if (*tok.code_points == 0) {
            ...
            if (tok.partial_utf8.n_remain != 0 &&
                !llama_grammar_match_partial_char(stack_pos,
                                                  tok.partial_utf8)) {
                rejects.push_back(tok);
            }
        } else if (llama_grammar_match_char(stack_pos, *tok.code_points)
                       .first) {
...
static std::vector<llama_grammar_candidate> llama_grammar_reject_candidates(
    ...
    for (size_t i = 1, size = stacks.size(); i < size; ++i) {
        rejects = llama_grammar_reject_candidates_for_stack(rules, stacks[i],
                                                            rejects);
    }
    return rejects;
}
...
struct llama_grammar *llama_grammar_copy(const struct llama_grammar *grammar) {
    llama_grammar *result = new llama_grammar{grammar->rules, grammar->stacks,
...
void llama_sample_top_p(struct llama_context *ctx,
    ...
    for (size_t i = 0; i < candidates->size; ++i) {
        ...
        // min_keep tokens we set the last index to i+1 to indicate that the
        // current iterate should be included in the set
...
void llama_sample_tail_free(struct llama_context *ctx,
    ...
    for (size_t i = 0; i < first_derivatives.size(); ++i) {
        first_derivatives[i] =
            candidates->data[i].p - candidates->data[i + 1].p;
    }
    ...
    for (size_t i = 0; i < second_derivatives.size(); ++i) {
        ...
        // Check if the running sum is greater than z or if we have kept at
...
void llama_sample_typical(struct llama_context *ctx,
    ...
    for (size_t i = 0; i < indices.size(); ++i) {
        ...
        // Check if the running sum is greater than typical or if we have kept
        // at least min_keep tokens
...
void llama_sample_repetition_penalty(struct llama_context *ctx,
    ...
    for (size_t i = 0; i < candidates->size; ++i) {
        const auto *token_iter =
            std::find(last_tokens, last_tokens + last_tokens_size,
                      candidates->data[i].id);
        if (token_iter == last_tokens + last_tokens_size) {
            continue;
        }

        // The academic publication that described this technique actually just
        // only divided, but that would cause tokens with negative logits to
        // become more likely, which is obviously wrong. This is common fix for
        // this problem, which is to multiply by the penalty instead of
        // dividing.
...
void llama_sample_grammar(struct llama_context *ctx,
    ...
    for (size_t i = 0; i < candidates->size; ++i) {
        ...
        if (id == eos) {
            ...
            candidates_decoded.push_back(
                decode_utf8(piece.c_str(), grammar->partial_utf8));
            candidates_grammar.push_back(
                {i, candidates_decoded.back().first.data(),
...
struct llama_beam {
    ...
    llama_beam_view view() const {
        return {tokens.data(), tokens.size(), p, eob};
    }
};
...
struct llama_logit_info {
    ...
    struct sum_exp {
        float max_l;
        float operator()(float sum, float l) const {
            return sum + std::exp(l - max_l);
        }
    };
    llama_logit_info(llama_context *ctx)
        : logits(llama_get_logits(ctx)), n_vocab(llama_n_vocab(ctx)),
          max_l(*std::max_element(logits, logits + n_vocab)),
          normalizer(1.0f / std::accumulate(logits, logits + n_vocab, 0.0f,
                                            sum_exp{max_l})) {
    }
    llama_token_data get_token_data(const llama_token token_id) const {
        constexpr auto p =
            std::numeric_limits<float>::quiet_NaN(); // never used
        return {token_id, logits[token_id], p};
    }
    // Return top k token_data by logit.
    std::vector<llama_token_data> top_k(size_t k) {
        std::vector<llama_token_data> min_heap; // min-heap by logit
        const llama_token k_min =
            std::min(static_cast<llama_token>(k), n_vocab);
...
struct llama_beam_search_data {
    ...
    llama_beam_search_data(llama_context *ctx, size_t n_beams, int n_past,
                           int n_predict, int n_threads)
        : ctx(ctx), n_beams(n_beams), n_past(n_past), n_predict(n_predict),
          n_threads(n_threads), beam_views(n_beams) {
    ...
    //  * Gather elements until the vector is full, then call std::make_heap()
    //  on it.
    //  * If the heap is full and a new element is found that should be
    //  included, pop the
    //    least element to the back(), replace it with the new, then push it
    //    into the heap.
    void fill_next_beams_by_top_probabilities(llama_beam &beam) {
        // Min-heaps use a greater-than comparator.
        const auto comp = [](const llama_beam &a, const llama_beam &b) {
            return a.p > b.p;
        };
        if (beam.eob) {
            // beam is at end-of-sentence, so just copy it to next_beams if its
            ...
            std::vector<llama_token_data> next_tokens =
                logit_info.top_k(n_beams);
            size_t i = 0;
            if (next_beams.size() < n_beams) {
                for (; next_beams.size() < n_beams; ++i) {
                    ...
                    next_beam.p *=
                        logit_info.probability_from_logit(next_tokens[i].logit);
            ...
            for (; i < n_beams; ++i) {
                const float next_p = beam.p * logit_info.probability_from_logit(
                                                  next_tokens[i].logit);
    ...
    size_t find_common_prefix_length() {
        ...
        for (size_t i = 1; i < beams.size(); ++i) {
            common_prefix_length =
                std::min(common_prefix_length, beams[i].tokens.size());
    ...
    llama_beams_state get_beams_state(const bool last_call) {
        ...
        return {beam_views.data(), beams.size(), common_prefix_length,
                last_call};
    }
    ...
    //  * the highest probability beam(s) (plural in case of ties) are not at
    //  end-of-sentence
    //    (since all other beam probabilities can only decrease)
    void loop(const llama_beam_search_callback_fn_t callback,
              void *const callback_data) {
        beams.push_back({{}, 1.0f, false}); // Start with one empty beam w/
                                            // probability = 1.0 and !eob.
        const auto not_eob = [](const llama_beam &beam) { return !beam.eob; };
        for (int i = 0; i < n_predict &&
                        std::any_of(beams.begin(), beams.end(), not_eob) &&
                        !beams[top_beam_index()].eob;
             ++i) {
            callback(callback_data,
                     get_beams_state(false)); // Sets common_prefix_length
            update_beams_from_beam_views();   // Update values (p,eob) that
                                              // callback may have changed.
            if (common_prefix_length) {
                llama_eval(ctx, beams[0].tokens.data(), common_prefix_length,
                           n_past, n_threads);
                n_past += common_prefix_length;
            }
            // Zero-out next_beam probabilities to place them last in following
            // min-heap.
            ...
            // next_beams become the beams of next/final iteration. Swap them to
            // re-use memory.
    ...
    static void renormalize_beam_probabilities(std::vector<llama_beam> &beams) {
        const auto sum_p = [](float sum, llama_beam &beam) {
            return sum + beam.p;
        };
        const float inv_sum =
            1.0f / std::accumulate(beams.begin(), beams.end(), 0.0f, sum_p);
        std::for_each(beams.begin(), beams.end(),
                      [=](llama_beam &beam) { beam.p *= inv_sum; });
    }
...
void llama_beam_search(llama_context *ctx,
                       ...
                       void *callback_data, size_t n_beams, int n_past,
                       int n_predict, int n_threads) {
    assert(ctx);
    const int64_t t_start_sample_us = ggml_time_us();
...
static void llama_convert_tensor_internal(struct ggml_tensor *tensor,
    ...
    if (ggml_is_quantized(tensor->type)) {
        ...
        throw std::runtime_error(
            format("cannot dequantize/convert tensor type %s",
                   ggml_type_name(tensor->type)));
    }

    if (nthread < 2) {
        if (tensor->type == GGML_TYPE_F16) {
            ggml_fp16_to_fp32_row((ggml_fp16_t *)tensor->data, f32_output,
                                  nelements);
    ...
    auto block_size = tensor->type == GGML_TYPE_F16
                          ? 1
                          : (size_t)ggml_blck_size(tensor->type);
    ...
    for (auto tnum = 0, in_buff_offs = 0, out_buff_offs = 0; tnum < nthread;
         tnum++) {
        auto thr_blocks =
            blocks_per_thread + (tnum == nthread - 1
                                     ? spare_blocks
                                     : 0); // num blocks for this thread
        auto thr_elems =
            thr_blocks * block_size; // number of elements for this thread
        auto thr_block_bytes =
            thr_blocks *
            block_size_bytes; // number of input bytes for this thread
        ...
        workers.emplace_back(compute, tensor->type,
                             (uint8_t *)tensor->data + in_buff_offs,
...
#ifdef GGML_USE_K_QUANTS
static ggml_type get_k_quant_type(ggml_type new_type, const ggml_tensor *tensor,
                                  const llama_model &model, llama_ftype ftype,
                                  int *i_attention_wv, int n_attention_wv,
                                  int *i_feed_forward_w2,
                                  int n_feed_forward_w2) {
    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
        ...
        if (model.type == MODEL_70B) {
            ...
            // As a result, the attn_v.weight tensor is 8x smaller compared to
            // attn_q.weight. Hence, we can get a nice boost in quantization
            // accuracy with nearly negligible increase in model size by
            // quantizing this tensor with more bits:
        ...
        if (ftype == LLAMA_FTYPE_MOSTLY_Q2_K) {
            ...
            new_type =
                *i_feed_forward_w2 < 2 ? GGML_TYPE_Q5_K
            ...
            new_type =
                model.arch == LLM_ARCH_FALCON ? GGML_TYPE_Q4_K : GGML_TYPE_Q5_K;
        } else if (ftype == LLAMA_FTYPE_MOSTLY_Q4_K_M) {
            if (model.arch == LLM_ARCH_FALCON) {
                new_type =
                    *i_feed_forward_w2 < 2 ? GGML_TYPE_Q6_K
                    : use_more_bits(*i_feed_forward_w2, n_feed_forward_w2)
                        ? GGML_TYPE_Q5_K
                        : GGML_TYPE_Q4_K;
            } else {
            ...
            new_type = GGML_TYPE_Q6_K;
                   ...
                   model.arch != LLM_ARCH_FALCON && *i_feed_forward_w2 < 4) {
            new_type = GGML_TYPE_Q5_K;
        }
        ++*i_feed_forward_w2;
    ...
    if (new_type == GGML_TYPE_Q2_K || new_type == GGML_TYPE_Q3_K ||
        ...
        int ny = tensor->ne[1];
        if (nx % QK_K != 0) {
            LLAMA_LOG_WARN("\n\n%s : tensor cols %d x %d are not divisible by "
                           "%d, required for k-quants\n",
                           __func__, nx, ny, QK_K);
    ...
    if (convert_incompatible_tensor) {
        if (name == tn(LLM_TENSOR_OUTPUT, "weight")) {
            new_type =
                GGML_TYPE_F16; // fall back to F16 instead of just failing.
...
static void
llama_model_quantize_internal(const std::string &fname_inp,
    ...
    if (n_attention_wv != n_feed_forward_w2 ||
        (uint32_t)n_attention_wv != model.hparams.n_layer) {
        LLAMA_LOG_WARN("%s ============ Strange model: n_attention_wv = %d, "
                       "n_feed_forward_w2 = %d, hparams.n_layer = %d\n",
                       __func__, n_attention_wv, n_feed_forward_w2,
    ...
    for (int i = 0; i < ml->n_tensors; ++i) {
        ...
        LLAMA_LOG_INFO("[%4d/%4d] %36s - [%s], type = %6s, ", ++idx,
                       ml->n_tensors, ggml_get_name(tensor),
...
#ifdef GGML_USE_K_QUANTS
            new_type = get_k_quant_type(new_type, tensor, model, ftype,
                                        &i_attention_wv, n_attention_wv,
                                        &i_feed_forward_w2, n_feed_forward_w2);
#endif
            // If we've decided to quantize to the same type the tensor is
            // already in then there's nothing to do.
        ...
        if (!quantize) {
            ...
            if (tensor->type == GGML_TYPE_F32) {
                f32_data = (float *)tensor->data;
                       ...
                       !params->allow_requantize) {
                throw std::runtime_error(
                    format("requantizing from type %s is disabled",
                           ggml_type_name(tensor->type)));
            } else {
                llama_convert_tensor_internal(tensor, f32_conv_buf, workers,
                                              nelements, nthread);
            ...
            if (nthread_use < 2) {
                ...
                size_t counter = 0;
                new_size = 0;
                auto compute = [&mutex, &counter, &hist_cur, &new_size,
                                new_type, f32_data, new_data, nelements]() {
                    ...
                    while (true) {
                        ...
                        if (first >= nelements) {
                            if (local_size > 0) {
                                for (int j = 0; j < int(local_hist.size());
                                     ++j) {
                        ...
                        local_size += ggml_quantize_chunk(
                            new_type, f32_data, new_data, first, last - first,
                            local_hist.data());
                    }
                };
...
int llama_apply_lora_from_file_internal(const struct llama_model &model,
    ...
    if (path_base_model) {
        ...
        if (ml->use_mmap) {
            ml->mapping.reset(
                new llama_mmap(&ml->file, /* prefetch */ 0, ggml_is_numa()));
        }
    }
        ...
        if (lora_tensors.find(base_name + ".loraA") != lora_tensors.end() &&
            ...
            if (dest_t->backend == GGML_BACKEND_GPU ||
                ...
                if (dest_t->type != GGML_TYPE_F16) {
                    throw std::runtime_error(format(
                        "%s: error: the simultaneous use of LoRAs and GPU "
                        "acceleration is only supported for f16 models",
                        __func__));
                }
                offload_func = ggml_cuda_assign_buffers;
                offload_func_force_inplace =
            ...
            if (ggml_is_quantized(base_t->type)) {
                if (!warned) {
                    LLAMA_LOG_WARN(
                        "%s: warning: using a lora adapter with a quantized "
                        "model may result in poor quality, "
            ...
            if (base_t->ne[0] != loraA->ne[1] ||
                base_t->ne[1] != loraB->ne[1]) {
                LLAMA_LOG_ERROR(
                    "%s: incompatible tensor dimensions (%" PRId64
                    " and %" PRId64 ");"
            ...
            // we won't need these tensors again, reset the context to save
            // memory
            ggml_free(lora_ctx);
...
llama_load_model_from_file(const char *path_model,
    ...
    if (!llama_model_load(
            path_model, *model, params.n_ctx, params.n_batch,
            params.n_gpu_layers, params.main_gpu, params.tensor_split,
            params.mul_mat_q, params.rope_freq_base, params.rope_freq_scale,
            params.low_vram, memory_type, params.use_mmap, params.use_mlock,
...
llama_new_context_with_model(struct llama_model *model,
    ...
    if (!params.vocab_only) {
        if (!llama_kv_cache_init(ctx->model.hparams, ctx->kv_self, memory_type,
                                 ctx->model.hparams.n_ctx,
                                 params.n_gpu_layers)) {
            LLAMA_LOG_ERROR(
                "%s: llama_kv_cache_init() failed for self-attention cache\n",
                __func__);
            llama_free(ctx);
            return nullptr;
        }

        {
            const size_t memory_size =
                ggml_nbytes(ctx->kv_self.k) + ggml_nbytes(ctx->kv_self.v);
...

        {
            ...
            // while the allocator buffer is used for the tensor data
            ...
            llama_token token = llama_token_bos(
                ctx); // not actually used by llama_build_graph, but required to
                      // choose between token and embedding inputs graph
            ggml_cgraph *gf =
                llama_build_graph(*ctx, &token, NULL, n_tokens, n_past);
#ifdef GGML_USE_METAL
            if (params.n_gpu_layers > 0) {
                ...
                ggml_allocr_set_parse_seq(
                    ctx->alloc, ggml_metal_get_concur_list(ctx->ctx_metal),
            ...
            ctx->alloc = ggml_allocr_new(ctx->buf_alloc.data,
                                         ctx->buf_alloc.size, tensor_alignment);
#ifdef GGML_USE_METAL
            if (ctx->ctx_metal) {
                ggml_allocr_set_parse_seq(
                    ctx->alloc, ggml_metal_get_concur_list(ctx->ctx_metal),
...
#ifdef GGML_USE_CUBLAS
            if (params.low_vram) {
                LLAMA_LOG_INFO("%s: not allocating a VRAM scratch buffer due "
                               "to low VRAM option\n",
                               __func__);
    ...
    if (!(result)) {                                                           \
        ...
        return NULL;                                                           \
            ...
            LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(
                ctx->ctx_metal, "data", data_ptr, data_size, max_size));

            LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(
                ctx->ctx_metal, "eval", ctx->buf_compute.data,
                ctx->buf_compute.size, 0));
            LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(
                ctx->ctx_metal, "kv", ctx->kv_self.buf.data,
                ctx->kv_self.buf.size, 0));

            LLAMA_METAL_CHECK_BUF(ggml_metal_add_buffer(
                ctx->ctx_metal, "alloc", ctx->buf_alloc.data,
                ctx->buf_alloc.size, 0));
...
int llama_model_desc(const struct llama_model *model, char *buf,
                     size_t buf_size) {
...
void llama_copy_state_data_internal(struct llama_context *ctx,
    ...
    // copy logits
    {
        ...
        if (padding_size > 0) {
            std::vector<uint8_t> padding(
                padding_size, 0); // Create a buffer filled with zeros
    ...
    // copy embeddings
    {
        ...
        if (embedding_size) {
            data_ctx->write(ctx->embedding.data(),
    ...
    // copy kv cache
    {
        ...
        if (kv_size) {
            ...
            ggml_context *cpy_ctx =
                ggml_init({4096, NULL, /* no_alloc */ true});
            ggml_cgraph gf{};
...
static bool llama_load_session_file_internal(struct llama_context *ctx,
    ...
    // sanity checks
    {
        ...
        if (session_hparams != ctx->model.hparams) {
            LLAMA_LOG_INFO(
                "%s : model hparams didn't match from session file!\n",
                __func__);
            return false;
        }
    }
    ...
    // load the prompt
    {
        ...
        if (n_token_count > n_token_capacity) {
            LLAMA_LOG_ERROR("%s : token count in session file exceeded "
                            "capacity! %u > %zu\n",
    ...
    // restore the context state
    {
        ...
        if (n_state_size_cur > n_state_size_max) {
            LLAMA_LOG_ERROR("%s : the state size in session file is too big! "
                            "max %zu, got %zu\n",
...
const char *llama_token_get_text(const struct llama_context *ctx,
                                 llama_token token) {
    return ctx->model.vocab.id_to_token[token].text.c_str();
}

float llama_token_get_score(const struct llama_context *ctx,
                            llama_token token) {
    return ctx->model.vocab.id_to_token[token].score;
}

llama_token_type llama_token_get_type(const struct llama_context *ctx,
                                      llama_token token) {
...
int llama_token_to_piece(const struct llama_context *ctx, llama_token token,
                         char *buf, int length) {
    return llama_token_to_piece_with_model(&ctx->model, token, buf, length);
}

// does not write null-terminator to buf
int llama_token_to_piece_with_model(const struct llama_model *model,
...
void llama_print_timings(struct llama_context *ctx) {
    ...
    LLAMA_LOG_INFO("%s:        load time = %8.2f ms\n", __func__,
                   timings.t_load_ms);
    LLAMA_LOG_INFO("%s:      sample time = %8.2f ms / %5d runs   (%8.2f ms per "
    ...
    LLAMA_LOG_INFO("%s: prompt eval time = %8.2f ms / %5d tokens (%8.2f ms per "
                   "token, %8.2f tokens per second)\n",
    ...
    LLAMA_LOG_INFO("%s:        eval time = %8.2f ms / %5d runs   (%8.2f ms per "
                   "token, %8.2f tokens per second)\n",
...
void llama_dump_timing_info_yaml(FILE *stream, const llama_context *ctx) {
    ...
    fprintf(
        ...
        ctx->n_eval);
    fprintf(stream,
            "n_p_eval: %d  # number of tokens processed in batches at the "
            "beginning\n",
            ctx->n_p_eval);
    ...
    fprintf(stream,
            "t_eval_us: %" PRId64
            "  # total microseconds spent generating tokens\n",
            ctx->t_eval_us);
    fprintf(stream,
            "t_load_us: %" PRId64
            "  # total microseconds spent loading the model\n",
            ctx->t_load_us);
    fprintf(stream,
            "t_p_eval_us: %" PRId64
            "  # total microseconds spent prompt processing\n",
            ctx->t_p_eval_us);
    fprintf(stream,
            "t_sample_us: %" PRId64 "  # total microseconds spent sampling\n",
            ctx->t_sample_us);
...
void llama_log_set(llama_log_callback log_callback, void *user_data) {
    g_state.log_callback =
        log_callback ? log_callback : llama_log_callback_default;
    g_state.log_callback_user_data = user_data;
}

static void llama_log_internal_v(llama_log_level level, const char *format,
                                 va_list args) {
    va_list args_copy;
