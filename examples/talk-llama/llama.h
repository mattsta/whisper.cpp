...
// '\n' and strip it if it exists. It might not exist for progress report where
// '.' is output repeatedly.
typedef void (*llama_log_callback)(enum llama_log_level level, const char *text,
                                   void *user_data);
...
llama_vocab_type(const struct llama_context *ctx);
...
// Returns 0 on success
LLAMA_API int llama_eval(struct llama_context *ctx, const llama_token *tokens,
                         int n_tokens, int n_past, int n_threads);

// Same as llama_eval, but use float matrix input directly.
LLAMA_API int llama_eval_embd(struct llama_context *ctx, const float *embd,
                              int n_tokens, int n_past, int n_threads);
...
LLAMA_API const char *llama_token_get_text(const struct llama_context *ctx,
                                           llama_token token);

LLAMA_API float llama_token_get_score(const struct llama_context *ctx,
                                      llama_token token);

LLAMA_API enum llama_token_type
llama_token_get_type(const struct llama_context *ctx, llama_token token);

// Special tokens
LLAMA_API llama_token
llama_token_bos(const struct llama_context *ctx); // beginning-of-sentence
LLAMA_API llama_token
llama_token_eos(const struct llama_context *ctx); // end-of-sentence
LLAMA_API llama_token
llama_token_nl(const struct llama_context *ctx); // next-line
...
// been returned
LLAMA_API int llama_tokenize(struct llama_context *ctx, const char *text,
                             llama_token *tokens, int n_max_tokens,
                             bool add_bos);

LLAMA_API int llama_tokenize_with_model(const struct llama_model *model,
                                        const char *text, llama_token *tokens,
...
LLAMA_API int llama_token_to_piece(const struct llama_context *ctx,
                                   llama_token token, char *buf, int length);

LLAMA_API int llama_token_to_piece_with_model(const struct llama_model *model,
...
struct llama_beams_state {
    ...
    size_t common_prefix_length; // Current max length of prefix tokens shared
                                 // by all beams.
...
LLAMA_API void llama_beam_search(struct llama_context *ctx,
                                 ...
                                 int n_past, int n_predict, int n_threads);
