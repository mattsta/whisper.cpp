...
bool whisper_params_parse(int argc, char **argv, whisper_params &params) {
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "-pms" || arg == "--prompt-ms") {
            params.prompt_ms = std::stoi(argv[++i]);
        } else if (arg == "-cms" || arg == "--command-ms") {
            params.command_ms = std::stoi(argv[++i]);
...
void whisper_print_usage(int /*argc*/, char **argv,
    ...
    fprintf(stderr,
            ...
            params.n_threads);
    fprintf(stderr,
            "  -pms N,     --prompt-ms N    [%-7d] prompt duration in "
            "milliseconds\n",
            params.prompt_ms);
    fprintf(stderr,
            "  -cms N,     --command-ms N   [%-7d] command duration in "
            "milliseconds\n",
            params.command_ms);
    fprintf(stderr, "  -c ID,      --capture ID     [%-7d] capture device ID\n",
            params.capture_id);
    fprintf(stderr,
            "  -mt N,      --max-tokens N   [%-7d] maximum number of tokens "
            "per audio chunk\n",
            params.max_tokens);
    fprintf(
        stderr,
...
uint64_t wait_for_vad(audio_async &audio, json jparams,
                      const whisper_params &params, uint64_t maxlength_ms,
                      std::vector<float> &pcmf32) {
    using namespace std::chrono;
    uint64_t time_now = time_point_cast<milliseconds>(system_clock::now())
                            .time_since_epoch()
                            .count();
    ...
    if (time_now - start_time < 500) {
        ...
        std::this_thread::sleep_for(
            milliseconds(500 - (time_now - start_time)));
        ...
        for (size_t offset = 0; offset < max_offset;
             offset += WHISPER_SAMPLE_RATE / 10) {
            std::vector<float> audio_chunk(
                &pcmf32[offset], &pcmf32[offset + WHISPER_SAMPLE_RATE]);
            if (::vad_simple(audio_chunk, WHISPER_SAMPLE_RATE, 1000,
    ...
    while (!::vad_simple(pcmf32, WHISPER_SAMPLE_RATE, 1000, params.vad_thold,
...
json unguided_transcription(struct whisper_context *ctx, audio_async &audio,
                            json jparams, const whisper_params &params) {
    std::vector<whisper_token> prompt_tokens;
    std::vector<float> pcmf32;
    uint64_t unprocessed_audio_timestamp =
        wait_for_vad(audio, jparams, params, 10000U, pcmf32);

    whisper_full_params wparams =
        whisper_full_default_params(WHISPER_SAMPLING_GREEDY);
    if (jparams.contains("prompt")) {
        // unlikely to see much use. Under normal circumstances, no_context
        // would be set to false
        std::string prompt = jparams.at("prompt");
        prompt_tokens.resize(1024);
        int n =
            whisper_tokenize(ctx, prompt.c_str(), prompt_tokens.data(), 1024);
...
json guided_transcription(struct whisper_context *ctx, audio_async &audio,
                          const whisper_params &params, json jparams,
                          std::vector<struct commandset> commandset_list) {
    struct commandset cs = commandset_list[jparams.value(
        "commandset_index", commandset_list.size() - 1)];
    std::vector<float> pcmf32;
    uint64_t unprocessed_audio_timestamp =
        wait_for_vad(audio, jparams, params, 2000U, pcmf32);
    ...
    // TODO: Do some time testing. Does an overly long prompt slow down
    // processing? Set up command sets/precompute prompts
    ...
    if (whisper_full(ctx, wparams, pcmf32.data(), pcmf32.size()) != 0) {
        ...
        throw json{
            {"code", -32803},
            {"message",
             "ERROR: whisper_full() failed"} // TODO: format string (sprintf?)
        };
    }
...
json register_commandset(struct whisper_context *ctx, json jparams,
                         std::vector<struct commandset> &commandset_list) {
    ...
    for (std::string s : jparams) {
        ...
        // The existing command implementation uses a nested for loop to
        // tokenize single characters I fail to see the purpose of this when '
        // a' has a wholly different pronunciation than the start of ' apple'
        ...
        if (!token_set.insert(tokens[0]).second) {
            fprintf(stderr,
                    "%s: warning: %s is a duplicate of an existing token\n",
                    __func__, s.c_str());
            throw json{{"code", -31000},
                       {"message", "Duplicate token in token set: " + s}};
        }
        if (n > 1) { // empty string if n=0? Should never occur
            fprintf(stderr,
                    "%s: error: command is more than a single token: %s\n",
                    __func__, s.c_str());
        }
    ...
    int n =
        whisper_tokenize(ctx, k_prompt.c_str(), cs.prompt_tokens.data(), 1024);
...
json seek(struct whisper_context * /*ctx*/, audio_async & /*audio*/,
          json /*params*/) {
    // whisper_state has the pertinent offsets, but there also seem to be a
    // large number of scratch buffers that would prevent rewinding context in a
    // manner similar to llama I'll give this a another pass once everything
    // else is implemented, but for now, it's unsupported
    throw json{{"code", -32601}, {"message", "Seeking is not yet supported."}};
}
json parse_job(const json &body, struct whisper_context *ctx,
               audio_async &audio, const whisper_params &params,
               std::vector<struct commandset> &commandset_list) {
    ...
    try {
        ...
        if (version != "2.0") {
            // unsupported version
            throw json{{"code", -3260}, {"message", "invalid jsonrpc version"}};
        }
        ...
        if (method == "unguided") {
            res = unguided_transcription(ctx, audio, jparams, params);
        } else if (method == "guided") {
            res = guided_transcription(ctx, audio, params, jparams,
                                       commandset_list);
        } else if (method == "seek") {
            res = seek(ctx, audio, jparams);
        } else if (method == "registerCommandset") {
            res = register_commandset(ctx, jparams, commandset_list);
        } else if (method == "echo") {
            res = jparams;
        }
        ...
        return json{{"jsonrpc", "2.0"}, {"error", ex}, {"id", id}};
    }
}

void process_loop(struct whisper_context *ctx, audio_async &audio,
                  const whisper_params &params) {
...
int main(int argc, char **argv) {
    ...
    audio.resume();
    // TODO: Investigate why this is required. An extra second of startup
    // latency is not great wait for 1 second to avoid any buffered noise
    std::this_thread::sleep_for(std::chrono::milliseconds(1000));
    audio.clear();
    // TODO: consider some sort of indicator to designate loading has finished?
    // Potentially better for the client to just start with a non-blocking
