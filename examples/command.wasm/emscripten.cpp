...
std::string command_transcribe(whisper_context *ctx,
...
void command_main(size_t index) {
    ...
    while (g_running) {
        ...
        if (ask_prompt) {
            ...
            {
                char txt[1024];
                snprintf(txt, sizeof(txt), "Say the following phrase: '%s'",
                         k_prompt.c_str());
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000, vad_thold,
                ...
                if (!have_prompt) {
                    command_get_audio(prompt_ms, WHISPER_SAMPLE_RATE,
                                      pcmf32_cur);

                    const auto txt = ::trim(::command_transcribe(
                        ctx, wparams, pcmf32_cur, prob0, t_ms));
                    ...
                    if (txt.length() < 0.8 * k_prompt.length() ||
...

                        {
                            ...
                            snprintf(txt, sizeof(txt),
                                     "Success! Waiting for voice commands ...");
                            command_set_status(txt);
                        }
                    ...
                    command_get_audio(command_ms, WHISPER_SAMPLE_RATE,
                                      pcmf32_cur);
                    ...
                    const auto txt = ::trim(::command_transcribe(
                        ctx, wparams, pcmf32_cur, prob, t_ms));
                    ...
                    {
                        char txt[1024];
                        snprintf(txt, sizeof(txt), "Command '%s', (t = %d ms)",
                                 command.c_str(), (int)t_ms);
                        command_set_status(txt);
                    }
                    {
                        std::lock_guard<std::mutex> lock(g_mutex);
...
EMSCRIPTEN_BINDINGS(command) {
    emscripten::function(
        "init",
        emscripten::optional_override([](const std::string &path_model) {
            for (size_t i = 0; i < g_contexts.size(); ++i) {
                if (g_contexts[i] == nullptr) {
                    ...
                    if (g_contexts[i] != nullptr) {
                        ...
                        g_worker = std::thread([i]() { command_main(i); });
    ...
    emscripten::function("free",
                         emscripten::optional_override([](size_t index) {
                             if (g_running) {
                                 g_running = false;
                             }
                         }));
    ...
    emscripten::function(
        "set_status",
        emscripten::optional_override([](const std::string &status) {
            {
                std::lock_guard<std::mutex> lock(g_mutex);
