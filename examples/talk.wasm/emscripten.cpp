...
void talk_main(size_t index) {
    ...
    while (g_running) {
        ...
        if (std::chrono::duration_cast<std::chrono::milliseconds>(t_now -
                                                                  t_last)
                .count() < step_ms) {
            {
                std::lock_guard<std::mutex> lock(g_mutex);
...

        {
            ...
            pcmf32 = std::vector<float>(
                g_pcmf32.end() -
                    std::min((int64_t)g_pcmf32.size(), window_samples),
                g_pcmf32.end());
        }
...

        {
            ...
            // remove all characters, except for letters, numbers, punctuation
            // and ':', '\'', '-', ' '
            ...
            talk_set_status("'" + text_heard +
                            "' - thinking how to respond (gpt-2) ...");

            const std::vector<gpt_vocab::id> tokens =
                gpt2_tokenize(g_gpt2, text_heard.c_str());
            ...
            if (tokens.size() > 0) {
                ...
                text_to_speak =
                    text_to_speak.substr(0, text_to_speak.find_first_of("\n"));
...
EMSCRIPTEN_BINDINGS(talk) {
            ...
            for (size_t i = 0; i < g_contexts.size(); ++i) {
                if (g_contexts[i] == nullptr) {
                    ...
                    if (g_contexts[i] != nullptr) {
                        ...
                        g_worker = std::thread([i]() { talk_main(i); });
    ...
    emscripten::function("force_speak",
                         emscripten::optional_override([](size_t index) {
                             {
                                 std::lock_guard<std::mutex> lock(g_mutex);
                                 g_force_speak = true;
    ...
    emscripten::function("get_text_context",
    ...
    emscripten::function("get_text_to_speak",
                         emscripten::optional_override([]() {
                             std::string text_to_speak;
    ...
    emscripten::function(
        "set_prompt",
        emscripten::optional_override([](const std::string &prompt) {
            {
                std::lock_guard<std::mutex> lock(g_mutex);
