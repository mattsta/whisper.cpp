...
bool whisper_params_parse(int argc, char **argv, whisper_params &params) {
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "-cmd" || arg == "--commands") {
            params.commands = argv[++i];
...
void whisper_print_usage(int /*argc*/, char **argv,
    ...
    fprintf(stderr,
            "  -cmd FNAME, --commands FNAME [%-7s] text file with allowed "
            "commands\n",
            params.commands.c_str());
    fprintf(stderr,
            "  -p,         --prompt         [%-7s] the required activation "
            "prompt\n",
            params.prompt.c_str());
    fprintf(stderr, "\n");
}
...
std::string transcribe(whisper_context *ctx, const whisper_params &params,
                       const std::vector<float> &pcmf32, float &prob,
                       int64_t &t_ms) {
...
int process_command_list(struct whisper_context *ctx, audio_async &audio,
                         const whisper_params &params) {
    fprintf(stderr, "\n");
    fprintf(stderr, "%s: guided mode\n", __func__);

    std::vector<std::string> allowed_commands =
        read_allowed_commands(params.commands);

    if (allowed_commands.empty()) {
        fprintf(stderr,
                "%s: error: failed to read allowed commands from '%s'\n",
    ...
    for (const auto &cmd : allowed_commands) {
        ...
        for (int l = 0; l < (int)cmd.size(); ++l) {
            ...
            //       the reason is that the first decoded token starts with a
            //       whitespace too!
    ...
    for (int i = 0; i < (int)allowed_commands.size(); ++i) {
        fprintf(stderr, "  - \033[1m%-*s\033[0m = [", max_len,
                allowed_commands[i].c_str());
    ...
    std::vector<whisper_token> k_tokens;
    {
        ...
        const int n =
            whisper_tokenize(ctx, k_prompt.c_str(), k_tokens.data(), 1024);
        if (n < 0) {
    ...
    while (is_running) {
        ...
        if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000,
            ...
            if (whisper_full(ctx, wparams, pcmf32_cur.data(),
                             pcmf32_cur.size()) != 0) {
            ...
            // NOTE: not optimal
            {
                ...
                // print the commands and the respective probabilities
                {
                    ...
                    for (const auto &cmd : probs_id) {
                        fprintf(stdout, "%s: %s%-*s%s = %f | ", __func__,
                                "\033[1m", max_len,
                                allowed_commands[cmd.second].c_str(), "\033[0m",
                                cmd.first);
                ...
                // best command
                {
                    ...
                    fprintf(
                        stdout,
                        "%s: detected command: %s%s%s | p = %f | t = %d ms\n",
                        __func__, "\033[1m", allowed_commands[index].c_str(),
                        "\033[0m", prob,
...
int always_prompt_transcription(struct whisper_context *ctx, audio_async &audio,
                                const whisper_params &params) {
    bool is_running = true;
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000,
                ...
                const auto txt =
                    ::trim(::transcribe(ctx, params, pcmf32_cur, prob, t_ms));
                ...
                if ((sim > 0.7f) && (command.size() > 0)) {
                    fprintf(stdout, "%s: Command '%s%s%s', (t = %d ms)\n",
                            __func__, "\033[1m", command.c_str(), "\033[0m",
                            (int)t_ms);
                }
...
int process_general_transcription(struct whisper_context *ctx,
                                  audio_async &audio,
                                  const whisper_params &params) {
    bool is_running = true;
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000,
                ...
                if (!have_prompt) {
                    ...
                    const auto txt = ::trim(
                        ::transcribe(ctx, params, pcmf32_cur, prob0, t_ms));
                    ...
                    // we have heard the activation phrase, now detect the
                    // commands
                    audio.get(params.command_ms, pcmf32_cur);
                    ...
                    const auto txt = ::trim(
                        ::transcribe(ctx, params, pcmf32_cur, prob, t_ms));
                    ...
                    float best_sim = 0.0f;
                    size_t best_len = 0;
                    for (int n = 0.8 * k_prompt.size();
                         n <= 1.2 * k_prompt.size(); ++n) {
