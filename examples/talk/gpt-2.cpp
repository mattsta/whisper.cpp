...
bool gpt2_model_load(const std::string &fname, gpt2_model &model,
    ...
    // computation
    ggml_type wtype =
        ggml_ftype_to_ggml_type((ggml_ftype)(model.hparams.ftype));
...

    {
        ...
        ctx_size +=
            n_layer * (n_embd * ggml_type_sizef(GGML_TYPE_F32)); // ln_1_g
        ctx_size +=
            n_layer * (n_embd * ggml_type_sizef(GGML_TYPE_F32)); // ln_1_b

        ctx_size +=
            n_layer * (n_embd * ggml_type_sizef(GGML_TYPE_F32)); // ln_2_g
        ctx_size +=
            n_layer * (n_embd * ggml_type_sizef(GGML_TYPE_F32)); // ln_2_b
        ...
        ctx_size += n_layer *
                    (n_embd * n_embd * ggml_type_sizef(wtype)); // c_attn_proj_w
        ctx_size += n_layer *
                    (n_embd * ggml_type_sizef(GGML_TYPE_F32)); // c_attn_proj_b

        ctx_size += n_layer * (4 * n_embd * n_embd *
                               ggml_type_sizef(wtype)); // c_mlp_fc_w
        ctx_size += n_layer *
                    (4 * n_embd * ggml_type_sizef(GGML_TYPE_F32)); // c_mlp_fc_b

        ctx_size += n_layer * (4 * n_embd * n_embd *
                               ggml_type_sizef(wtype)); // c_mlp_proj_w
        ctx_size +=
    ...
    // prepare memory for the weights
    {
        ...
        for (int i = 0; i < n_layer; ++i) {
            ...
            layer.c_attn_attn_b =
                ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 3 * n_embd);
            ...
            layer.c_mlp_fc_w =
                ggml_new_tensor_2d(ctx, wtype, n_embd, 4 * n_embd);
            layer.c_mlp_fc_b =
                ggml_new_tensor_1d(ctx, GGML_TYPE_F32, 4 * n_embd);

            layer.c_mlp_proj_w =
                ggml_new_tensor_2d(ctx, wtype, 4 * n_embd, n_embd);
            layer.c_mlp_proj_b = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, n_embd);

            // map by name
            model.tensors["model/h" + std::to_string(i) + "/ln_1/g"] =
                layer.ln_1_g;
            model.tensors["model/h" + std::to_string(i) + "/ln_1/b"] =
                layer.ln_1_b;

            model.tensors["model/h" + std::to_string(i) + "/ln_2/g"] =
                layer.ln_2_g;
            model.tensors["model/h" + std::to_string(i) + "/ln_2/b"] =
                layer.ln_2_b;
    ...
    // load weights
    {
        ...
        while (true) {
            ...
            if (0) {
                printf(
                    "%24s - [%5d, %5d], type = %6s, %6.2f MB, %9zu bytes\n",
...
bool gpt2_eval(const gpt2_model &model, const int n_threads, const int n_past,
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // [2304, N]
        {
            ...
            cur = ggml_add(
                ctx0, ggml_repeat(ctx0, model.layers[il].c_attn_attn_b, cur),
                cur);
        }

        // self-attention
        {
            struct ggml_tensor *Qcur = ggml_view_2d(
                ctx0, cur, n_embd, N, cur->nb[1], 0 * sizeof(float) * n_embd);
            struct ggml_tensor *Kcur = ggml_view_2d(
                ctx0, cur, n_embd, N, cur->nb[1], 1 * sizeof(float) * n_embd);
            struct ggml_tensor *Vcur = ggml_view_2d(
                ctx0, cur, n_embd, N, cur->nb[1], 2 * sizeof(float) * n_embd);
        ...
        // [768, N]
        {
            ...
            cur = ggml_add(
                ctx0, ggml_repeat(ctx0, model.layers[il].c_attn_proj_b, cur),
                cur);
        }
        ...
        // feed-forward network
        {
            ...
            cur = ggml_add(
                ctx0, ggml_repeat(ctx0, model.layers[il].c_mlp_fc_b, cur), cur);
            ...
            cur = ggml_add(
                ctx0, ggml_repeat(ctx0, model.layers[il].c_mlp_proj_b, cur),
                cur);
        }
    ...
    // norm
    {
        ...
        // [ 768, N]
        inpL = ggml_add(
            ctx0, ggml_mul(ctx0, ggml_repeat(ctx0, model.ln_f_g, inpL), inpL),
...
Me too.
    ...
    int32_t n_threads =
        std::min(N_THREAD, (int)std::thread::hardware_concurrency());
...
std::string gpt2_gen_text(gpt2_context *ctx, const char *text, int max_tokens) {
    ...
    int n_predict =
        std::min(max_tokens, ctx->model.hparams.n_ctx - (int)embd_inp.size());
    ...
    for (int i = embd.size(); i < (int)embd_inp.size() + n_predict; i++) {
        // predict
        if (!embd.empty()) {
            if (!gpt2_eval(ctx->model, ctx->n_threads, n_past, embd, embd_w,
                           mem_per_token)) {
...

        {
            ...
            const gpt_vocab::id id = gpt_sample_top_k_top_p(
                ctx->vocab, embd_w.data() + (embd_w.size() - n_vocab), top_k,
                top_p, temp, ctx->rng);
