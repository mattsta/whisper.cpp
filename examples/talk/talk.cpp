...
bool whisper_params_parse(int argc, char **argv, whisper_params &params) {
    for (int i = 1; i < argc; i++) {
        ...
        } else if (arg == "-mg" || arg == "--model-gpt") {
            params.model_gpt = argv[++i];
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1250,
                ...
                if (!force_speak) {
                    text_heard = ::trim(
                        ::transcribe(ctx_wsp, params, pcmf32_cur, prob0, t_ms));
                }
                ...
                const std::vector<gpt_vocab::id> tokens =
                    gpt2_tokenize(ctx_gpt, text_heard.c_str());
                ...
                fprintf(stdout, "%s: Heard '%s%s%s', (t = %d ms)\n", __func__,
                        "\033[1m", text_heard.c_str(), "\033[0m", (int)t_ms);
...

                {
                    ...
                    std::string prompt =
                        ::replace(::replace(k_prompt, "{0}", params.person),
                                  "{1}", prompt_base);

                    text_to_speak = gpt2_gen_text(ctx_gpt, prompt.c_str(),
                                                  params.max_tokens);
                    text_to_speak = std::regex_replace(
                        text_to_speak,
                        std::regex("[^a-zA-Z0-9\\.,\\?!\\s\\:\\'\\-]"), "");
                    text_to_speak = text_to_speak.substr(
                        0, text_to_speak.find_first_of('\n'));
                ...
                text_to_speak =
                    ::replace(text_to_speak, params.person + ": ", "");
                int ret =
