...
void gpt_print_usage(int /*argc*/, char **argv, const gpt_params &params) {
    ...
    fprintf(stderr,
            "  -b N, --batch_size N  batch size for prompt processing "
            "(default: %d)\n",
            params.n_batch);
            ...
            if (!in_token) {
                ...
                if (json[i] == '\\' && i + 1 < n) {
                    ...
                    try {
                        ...
                        // fprintf(stderr, "%s: ignoring key '%s' with value
    ...
    for (const auto &word : words) {
        for (int i = 0; i < (int)word.size();) {
            for (int j = word.size() - 1; j >= i; j--) {
                ...
                if (it !=
                    vocab.token_to_id.end()) { // word.substr(i, j-i+1) in vocab
...
std::vector<gpt_vocab::id> parse_tokens_from_string(const std::string &input,
                                                    char delimiter) {
...
extract_tests_from_file(const std::string &fpath_test) {
    ...
    while (std::getline(fin, line)) {
        ...
        if (delimiterPos != std::string::npos) {
            ...
            std::string s_tokens =
                line.substr(delimiterPos + std::strlen(delimeter));
...
gpt_sample_top_k_top_p_repeat(const gpt_vocab &vocab, const float *logits,
...

    {
        ...
        for (int i = 0; i < n_logits; ++i) {
            ...
            // (https://arxiv.org/abs/1909.05858) credit
            // https://github.com/facebookresearch/llama/compare/main...shawwn:llama:main
            if (repeat_last_n > 0 &&
                std::find(last_n_tokens.end() - repeat_last_n,
                          last_n_tokens.end(), i) != last_n_tokens.end()) {
                // if score < 0 then repetition penalty has to multiplied to
                // reduce the previous token probability
                if (plogits[i] < 0.0f) {
                    logits_id.push_back(
...
bool vad_simple(std::vector<float> &pcmf32, int sample_rate, int last_ms,
    ...
    if (verbose) {
        fprintf(stderr,
                "%s: energy_all: %f, energy_last: %f, vad_thold: %f, "
                "freq_thold: %f\n",
